...
function Respawn()
	...
	if not TestValid(last_droid) then
		unit_list = Find_All_Objects_Of_Type("Spawned_Repairdroid")
		if table.getn(unit_list) < 6 then 
			Object.Play_SFX_Event("Unit_AT_AT_Rope_Drop")
			last_droid = Create_Generic_Object("Spawned_Repairdroid",Object.Get_Position(),rebel_player)
